...
class TestPartialDemangleNameChopping:
    ...
    @pytest.mark.skipif(V < 20, reason="Fixed-point types require LLVM >= 20")
    @pytest.mark.parametrize(
        "mangled,context,base_name,return_type,params",
        FIXED_POINT_DATA,
...
class TestItaniumDemangle:
    ...
    def test_basic(self):
        assert llvmdemangle.itanium_demangle("_Z3fooi") == "foo(int)"

    def test_invalid(self):
        assert llvmdemangle.itanium_demangle("not_itanium") is None

    def test_namespace(self):
        assert llvmdemangle.itanium_demangle("_ZN3Foo3barEid") == \
            "Foo::bar(int, double)"

    def test_template(self):
        assert llvmdemangle.itanium_demangle("_ZN3Foo3barIiEEvi") == \
            "void Foo::bar<int>(int)"

    @pytest.mark.skipif(V < 18, reason="parse_params requires LLVM >= 18")
    def test_parse_params_false(self):
        assert llvmdemangle.itanium_demangle("_Z3fooi", parse_params=False) == "foo"

    @pytest.mark.skipif(V < 18, reason="parse_params requires LLVM >= 18")
    def test_parse_params_true(self):
        assert llvmdemangle.itanium_demangle("_Z3fooi", parse_params=True) == "foo(int)"
